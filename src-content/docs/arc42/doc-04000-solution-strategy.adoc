...
| Single source profile knowledge | xref:qg-002-maintainability[QG-002], xref:qg-004-correctness[QG-004] | Profile artifacts are authored once and reused across website, CV, README, and article exports.
| Metadata-backed content | xref:qg-002-maintainability[QG-002], xref:qg-004-correctness[QG-004] | Standalone content artifacts receive front matter; include-heavy reusable fragments are covered by sidecar metadata.
| Toolkit-compatible architecture knowledge | xref:qg-002-maintainability[QG-002], xref:qg-003-automation[QG-003] | arc42 chapters, ADRs, quality scenarios, risks, schemas, validators, and generated fragments follow architecture-knowledge-toolkit conventions.
| Gradle task orchestration | xref:qg-001-reproducibility[QG-001], xref:qg-003-automation[QG-003] | Build, validation, and generation are exposed as repeatable Gradle tasks.
| Containerized CI/CD | xref:qg-001-reproducibility[QG-001], xref:qg-005-sustainability[QG-005] | GitHub Actions run the same toolchain in the docs-toolbox container and deploy changed outputs through target-specific deploy jobs.
| Scoped deployment credentials | xref:qg-005-sustainability[QG-005] | Deployment credentials are separated by target. GitHub profile README deployment uses a fine-grained token for `dieterbaier/dieterbaier`; GitLab README deployment uses a GitLab token; private webspace deployment uses SFTP credentials.
|===
