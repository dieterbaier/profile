...
// This article's own assets — see the German original for what the two lines do.
// A translation owns its directory the same way an original does: it is named
// after this artifact's ID, not the ID it was translated from, so a diagram that
// carries language keeps a per-language source and a per-language rendering.
:assetsdir: ART-003-doc-as-code-en
:imagesdir: {assetsdir}
...
plantuml::{docfile}/../{assetsdir}/doc-influences-en.puml[format=svg, alt="Forces acting on documentation",width=100%]
...
plantuml::{includesdir}/diagrams/doc-pipeline.puml[format=svg, alt="Documentation build pipeline",width=100%]
