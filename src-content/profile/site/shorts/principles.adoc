= Dieter Baier
ifdef::buildsite[]
:basedir: ..
endif::[]
include::{includesdir}/../../revinfo.adoc[]
:author!: Dieter Baier
:author_page: {basedir}/index.html
:author_page: index.html
:revremark: Pragmatic Architecture Principles
:revdate!: 03/2023
:revnumber!: 1.0
:active: shorts
include::{includesdir}/docheader.adoc[]

siehe auch: xref:{basedir}/shorts/architecture-thinking.adoc[Architekturgedanken]

toc::[]

== Clear Ownership
[.highlight.info]
Every business capability should have a clear owner.

== Loose Coupling
[.highlight.info]
Systems should evolve independently.

== High Cohesion
[.highlight.info]
Components should have a focused responsibility.

== Minimize Ripple Effects
[.highlight.info]
Changes should affect as few components as possible.

== Explicit Boundaries
[.highlight.info]
System boundaries must be visible.

== Understandability
[.highlight.info]
Systems must remain understandable.

== Evolutionary Architecture
[.highlight.info]
Architecture must support change.

== Explicit Decisions
[.highlight.info]
Important architectural decisions should be documented.

== Static where possible
[.smaller.italic]
Primarily (but not exclusively) with a focus on web design - for the *environmental impact*

[.highlight.info]
Dynamic only where necessary.
