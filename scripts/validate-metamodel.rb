...
class OpenQuestionsIndexGenerator
  ...
  def initialize(root:, docs_dir:, questions_path: nil)
    @root = Pathname.new(root).expand_path
    @docs_dir = output_base(docs_dir)
    default_questions_path = @root.join('src-content/docs/doc-005-questions-and-answers.adoc')
    default_questions_path = @root.join('src/docs/doc-005-questions-and-answers.adoc') unless default_questions_path.exist?
    @questions_path = Pathname.new(questions_path || default_questions_path).expand_path
    @output_paths = []
  end
